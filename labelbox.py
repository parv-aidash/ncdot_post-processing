...
class shpDataExtractor:
    ...
    def batch_scheduler(self, tool):
        ...
        all_files = self.list_files(f"ncdot/final_data/{tool}/shapefiles/lz/")
    ...
    for dirpath, subdirs, files in os.walk(path):
        for x in files:
            if x.endswith("_poly.shp"):
                poly_shpfiles.append(os.path.join(dirpath, x))
                
    sdf = pd.concat([
        gpd.read_file(pshp)
        for pshp in poly_shpfiles
    ]).pipe(gpd.GeoDataFrame)
...
#####
    ...
    point_target_shp_name = 'county'+'_DS_'+str(dt)+'_missionBatch_LB_point.shp'
    point_target_shp_file_folder = 'county'+'_DS_'+str(dt)+'_missionBatch_LB_point'
    poly_target_shp_name = 'county'+'_DS_'+str(dt)+'_missionBatch_LB_poly.shp'
    poly_target_shp_file_folder = 'county'+'_DS_'+str(dt)+'_missionBatch_LB_poly'
    ...
    q."images", q."bboxes", q."lat", q."lon", q."geohash",
...
#, foo."UniqueID"
    ...
    if len(point_df_all) > 0 or len(poly_df_all) > 0:
        ...
        final_df['Area'] = final_df['geometry'].to_crs('epsg:32633').map(lambda p: p.area)
